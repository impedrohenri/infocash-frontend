...
export default function FormRegistro(props) {
    ...
    return (
        ...
        <form id={props.id} onSubmit={props.onSubmit} className={`modal-body ${styles.modal_Body}`}>
            ...
            <select className="form-select my-2" name='categoria' id="selectCategoria"
                onChange={() => { setSelectedCategoria(document.getElementById("selectCategoria").value) }} required>
            ...
            <div className={`d-flex row justify-content-around py-4 mx-1 ${styles.divRadios}`}>
                ...
                <label className={`${styles.recorrencia}`} htmlFor='unica_id'>
                    <input className="my-auto" type="radio" name="recorrencia" id="unica_id" value='nao' defaultChecked={true} />Operação única
                </label>

                <label className={`${styles.recorrencia}`} htmlFor='recorrente_id'>
                    <input className="my-auto" type="radio" name="recorrencia" id="recorrente_id" value='sim' />
                    Operação recorrente
                </label>
